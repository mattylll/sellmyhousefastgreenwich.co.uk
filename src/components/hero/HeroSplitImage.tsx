...
import { Star, Search, Phone } from "lucide-react";
...
export default function HeroSplitImage() {
  ...
  function handlePostcodeSubmit(e: React.FormEvent) {
    ...
    if (trimmed) {
      // Fire a lightweight lead with just the postcode — non-blocking
      submitLead({ formType: "hero-postcode", postcode: trimmed });
      // Store postcode so ValuationForm can pre-fill it
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-gray-900">
      {/* Background - row of houses aerial photo */}
      <img
        src={config.images.hero}
        alt={"Houses in " + config.location.name}
