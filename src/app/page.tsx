...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* Meet Your Local Buyer — human element */}
      <section className="py-16 md:py-24 bg-[var(--color-card)]">
        <div className="container mx-auto px-4 max-w-6xl">
          <div className="grid md:grid-cols-2 gap-12 lg:gap-16 items-center">
            {/* Image */}
            <div className="relative">
              <div className="aspect-[4/5] rounded-2xl overflow-hidden shadow-xl border border-[var(--color-border)]/30">
                <img
                  src={config.images.team || config.images.about}
                  alt={'Your local property buyer in ' + config.location.name}
                  className="w-full h-full object-cover"
                  loading="lazy"
                />
              </div>
              {/* Floating stat card */}
              <div className="absolute -bottom-6 -right-4 md:-right-8 bg-[var(--color-background)] rounded-xl shadow-lg border border-[var(--color-border)] p-5 md:p-6">
                <p className="text-3xl md:text-4xl font-extrabold text-[var(--color-accent)] leading-none">{config.brand.housesBought}+</p>
                <p className="text-sm text-[var(--color-muted-foreground)] mt-1 font-medium">Properties purchased</p>
              </div>
            </div>
            ...
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-bold text-[var(--color-foreground)] leading-tight mb-6">
                Your Trusted Local Cash Buyer in {config.location.name}
              </h2>
              <p className="text-lg text-[var(--color-muted-foreground)] leading-relaxed mb-6">
                {config.content.aboutStory.slice(0, 300)}{config.content.aboutStory.length > 300 ? '...' : ''}
              </p>

              <div className="space-y-4 mb-8">
                {[
                  { stat: config.brand.yearsExperience + '+ years', label: 'buying properties across ' + config.location.region },
                  { stat: config.brand.averageDaysToComplete + ' day', label: 'average completion time' },
                  { stat: '£0', label: 'fees — we cover all costs including legal' },
                ].map((item) => (
                  <div key={item.label} className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-[var(--color-accent)]/10 flex items-center justify-center shrink-0">
                      <span className="text-sm font-bold text-[var(--color-accent)]">{item.stat}</span>
                    </div>
                    <span className="text-[var(--color-muted-foreground)] text-base capitalize">{item.label}</span>
                  </div>
                ))}
              </div>
              ...
              <a
                href="/about"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[var(--color-accent)] text-white font-semibold rounded-xl hover:opacity-90 transition-all shadow-lg shadow-[var(--color-accent)]/25"
              >
                Learn More About Us
      ...
      {config.layout.sectionsOrder
        .filter((key) => key !== 'hero')
        .filter((key) => !config.layout.hiddenSections?.includes(key))
        .map((sectionKey) => {
