...
export function generateLocalBusinessSchema(config: SiteConfig) {
  return {
    ...
    '@type': 'LocalBusiness',
    '@id': `https://${config.brand.domain}/#organization`,
    ...
    aggregateRating: {
      ...
      ratingValue: '4.9',
      reviewCount: String(config.content.testimonials.length),
      bestRating: '5',
    },
    priceRange: '££',
  };
...
export function generateWebSiteSchema(config: SiteConfig) {
  return {
    ...
    potentialAction: {
      '@type': 'SearchAction',
      target: `https://${config.brand.domain}/areas/{search_term_string}`,
